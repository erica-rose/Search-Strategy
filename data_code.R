...
##set up timer (taken from http://stackoverflow.com/questions/10893611/storing-tic-toc-values-in-r)
...
initialinfective <- 18 #set this house as initialinfective
id=1:N #generate ids
...
#     accept.Rb[m] <- 0}
  ...
  if(add.del.move=="move"){
    ...
    addinf<-which(add.house==0 & I==Inf&(inspected==0 | (inspected == 1 & (tobs<(maxt-2) ))))
    if(length(addinf)>1){
  ...
  if(m%%100000==0) {
  	print(m)
  	write.csv(occult.prob.ids.ordered, file=paste("/home/ebillig/Jewell_data/Juan4/beta",betastart,"Juan4.Oct21.csv", sep=""))
    save.image(paste("/home/ebillig/Jewell_data/Juan4/beta",betastart,"Juan4.Oct21.Rdata", sep=""))}
# if(m%%100==0){
#   print(m)
#   print(N_I)
#   print(beta[m])
